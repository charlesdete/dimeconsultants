...
function HomePage() {
  return <PageLayout>
    ...
    <Section><div className="logo-heading"><Eyebrow>Trusted across industries</Eyebrow><h2 className="section-title">Partners and clients we've worked with</h2></div><div className="logo-grid">{partners.map((p) => <div key={p.name} className="logo-cell"><img src={p.src} alt={`${p.name} logo`} loading="lazy" /></div>)}</div></Section>
