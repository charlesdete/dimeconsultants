    ...
    <PageLayout>
      ...
      <section className="relative overflow-hidden border-b border-border" style={{ background: "var(--gradient-hero)" }}>
        ...
        <div className="relative mx-auto flex max-w-7xl flex-col items-center px-6 pb-28 pt-24 text-center md:pt-32">
          ...
          <motion.h1
            ...
            className="text-balance text-5xl font-semibold leading-[1.05] md:text-7xl"
          >
          ...
          <div className="mt-20 grid w-full grid-cols-2 gap-px overflow-hidden rounded-2xl border border-border bg-border md:grid-cols-4">
            {stats.map((s) => (
              <div key={s.l} className="bg-card p-6 text-left">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {services.map((s) => (
            <motion.div
              ...
              className="group relative overflow-hidden rounded-2xl border border-border bg-card p-6 transition hover:border-cyan/50"
            >
          ...
          <div>
            ...
            <ul className="mt-6 space-y-3 text-sm">
              {["AI Foundations","Agents & Automation","Practical Tool Mastery","Ethics & Compliance","AI Strategy for Leaders"].map(m => (
                <li key={m} className="flex items-center gap-3"><CheckCircle2 size={18} className="text-cyan"/>{m}</li>
              ))}
            </ul>
            ...
            <div className="glass relative rounded-3xl p-8">
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={x.t} className="flex items-center gap-3 rounded-xl border border-border bg-card/60 p-4">
      ...
      <Section>
        ...
        <div className="grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-border bg-border sm:grid-cols-3 md:grid-cols-6">
          {["Kuehne+Nagel","Habib Bank","Sigma Tech","Equator Health","Nyumbani Group","Savannah Logistics"].map(n => (
            <div key={n} className="flex h-24 items-center justify-center bg-card px-4 text-center text-sm text-muted-foreground">{n}</div>
          ))}
        </div>
      </Section>
