    ...
    return (<PageLayout>
      <Section>
        ...
        <div className="mt-14 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {posts.map((p) => (<article key={p.t} className="group flex flex-col rounded-2xl border border-border bg-card p-6 transition hover:border-cyan/50">
      ...
      <section className="border-y border-border bg-[var(--deep)]">
        <Section className="!py-20">
          <div className="grid gap-10 md:grid-cols-2 md:items-center">
            ...
            <form onSubmit={(e) => { e.preventDefault(); setDone(true); }} className="flex flex-col gap-3 rounded-2xl border border-border bg-card p-6 sm:flex-row">
              <input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@company.com" className="flex-1 rounded-lg border border-border bg-background px-4 py-3 text-sm outline-none focus:border-cyan"/>
