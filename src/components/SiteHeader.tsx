  ...
  return (
    <header className="sticky top-0 z-40 glass">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6">
        ...
        <nav className="hidden items-center gap-1 lg:flex">
          {NAV.map((n) => (
            <Link
              ...
              activeProps={{ className: "rounded-md px-3 py-2 text-sm text-cyan font-medium" }}
            >
              {n.label}
            </Link>
          ))}
        ...
        <Link
          to="/contact"
          className="hidden rounded-full bg-cyan px-5 py-2 text-sm font-semibold text-primary-foreground shadow-[0_0_20px_oklch(0.78_0.16_210/0.5)] transition hover:opacity-90 lg:inline-flex"
        >
          Book Discovery Call
        </Link>
