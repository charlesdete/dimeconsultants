  ...
  return (
    <PageLayout>
      <Section>
        ...
        <h1 className="page-title">Field notes from the AI frontier.</h1>
        <p className="page-copy">
          Articles, playbooks and downloads from our work with teams around the world.
        </p>
        <div className="responsive-grid three insights-grid">
          {posts.map((p) => (
            <article key={p.t} className="card insight-card">
              <span className="insight-tag">{p.tag}</span>
              <h3>{p.t}</h3>
              <p>{p.d}</p>
              <Link to="/insights" className="text-link">Read <ArrowRight size={14} /></Link>
            </article>
          ))}
        </div>
      </Section>

      <section className="split-band download-section">
        <Section>
          <div className="download-grid">
            <div>
              <Eyebrow>Free download</Eyebrow>
              <h2 className="section-title">The AI-Ready Workforce Field Guide</h2>
              <p className="section-copy">
                A 24-page playbook on building AI capability inside any organisation. Get the PDF
                in your inbox.
              </p>
            </div>
            <form
