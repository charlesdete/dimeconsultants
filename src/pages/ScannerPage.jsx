...
export function ScannerPage() {
  ...
  const tier =
    pct >= 75
      ? { name: "AI Accelerator", note: "You're ahead of the curve. Time to scale agentic automations across the enterprise." }
      : pct >= 45
      ? { name: "AI Builder", note: "Strong foundations. A structured 90-day programme will compound your gains." }
      : { name: "AI Starter", note: "Big upside ahead. Start with foundations, governance and a focused pilot." };
  ...
  return (
    <PageLayout>
      <Section>
        ...
        <h1 className="page-title">AI Readiness Scanner</h1>
        <p className="page-copy">
          Answer 6 quick questions. Get an instant readiness tier and recommended next steps.
        </p>
        <div className="scanner-panel">
          {!done ? (
            <>
              ...
              <h3 className="scanner-question">{QUESTIONS[step].q}</h3>
              <div className="answer-grid">
                {QUESTIONS[step].o.map((o, i) => (
                  <button
                    key={o}
                    onClick={() => {
            ...
            <div className="scanner-result">
              <div className="result-label">Your readiness tier</div>
              <div className="result-tier">{tier.name}</div>
              <div className="result-score">{pct}/100</div>
              <p className="result-note">{tier.note}</p>
              <div className="result-actions">
                <a href="/contact" className="button-primary">
                  Discuss your results <ArrowRight size={16} />
                </a>
